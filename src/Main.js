...
export default function Main() {
  ...
  const handleNext = () => {
    ...
    if (activeStep === 0) {
      ...
      if (!port || !port.trim()) {
        setPortError('Please enter Port');
        ret = false;
      }
      if (!religion) {
      ...
      if (!edu || !edu.trim()) {
        setEduError('Please enter Education Level');
        ret = false;
      }
    }
  ...
  const handleSubmit = async event => {
    ...
    await axios.post(`http://localhost:5001/candidate`, {
      id, photo, linkVideo, passportStatus: passport, bioFee, loan, pocketMoney, minimumSalary, maritalStatus,
      ...
      .then(res => {
        ...
        axios.post(`http://localhost:5001/medicalHistory`, {
          id, allergies, mentalIllness: mental, epilepsy, asthma, diabetes, hypertension: hyper, tubercolosis: tbc, heartDisease: heart,
          malaria, operations, otherIllness: othersIll, disabilities, diet, pork, beef, otherFoods: othersFood, preference: diet,
          remarkFood: remar,
        })
          .then(res => {
            console.log(res);
            console.log(res.data);
            axios.post(`http://localhost:5001/skillsSEA`, {
              ...
              .then(res => {
                console.log(res);
                console.log(res.data);
                axios.post(`http://localhost:5001/skillsTCEA`, {
