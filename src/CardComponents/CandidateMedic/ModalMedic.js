...
export default function ModalMedic(props) {
    const { candidateMedic, setCandidateMedic } = props;
    ...
    const handleSubmit = async event => {
        ...
        await axios.put(`http://localhost:5001/alexmedical/${candidateMedic.id}`, {
            allergies, mentalIllness: mental, epilepsy, asthma, diabetes, hypertension: hyper, tubercolosis: tbc, heartDisease: heart,
            malaria, operations, otherIllness: othersIll, disabilities, diet, pork, beef, otherFoods: othersFood
        })
            .then(res => {
                console.log(res.data)

                const result = res.data;
                setCandidateMedic(result);
